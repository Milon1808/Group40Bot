...
public sealed class RoleModule(ISettingsStore store) : InteractionModuleBase<SocketInteractionContext>
{
        ...
        bool remove_on_unreact = true)
    {
        ...
        void Add(SocketRole? role, string? emoji)
        {
            ...
            if (role.Position >= me.Hierarchy) throw new InvalidOperationException($"Bot role must be higher than @{role.Name}.");
        ...
        foreach (var p in pairs) sb.AppendLine($"{p.Raw} -> {p.Role.Mention}");
        sb.AppendLine("```");
